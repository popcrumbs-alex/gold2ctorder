  ...
  const handleAddOTOToOrder = async (oto: OtoOptionProps) => {
    ...
    try {
      ...
      if (request.data.updateOrder.success) {
        navigate("/victoria/otos/RoyaltyRing");
      }
    } catch (error) {
      console.error(error);
      return error;
            ...
            paypal: (
              <ContinueBox>
                ...
                <Paypal
                  ...
                  nextPage={"/victoria/otos/RoyaltyRing"}
                  fromOrderPage={false}
                  items={[
                    {
...
export default EternityBandScreen;
